-- ─────────────────────────────────────────────────────────────────────────
-- Moderator flag + badge plumbing through post/comment RPCs.
-- ─────────────────────────────────────────────────────────────────────────

alter table public.profiles add column if not exists is_moderator boolean not null default false;

-- Grant moderator to the owner account.
update public.profiles set is_moderator = true where username = 'tofuyu';

-- ── Re-assert post RPCs to include author_is_moderator ──
...
language sql stable security definer set search_path = public
as $$
  select c.id, c.content, c.created_at, a.username, a.display_name, a.avatar_url, a.is_verified, a.is_moderator
