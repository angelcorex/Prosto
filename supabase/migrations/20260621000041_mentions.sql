...
--
-- Re-assert send_dm so that, on top of the normal 'message' notification, a
-- 'mention' notification (which lights up the bell badge) is created for the
-- people a message actually pings:
--   • @everyone or @here  → every other participant
...
begin
  ...
  -- Plain "message" notification (DM list lights up; not the bell).
  ...
  if body ~* '@(everyone|here)([^a-z0-9_]|$)' then
    insert into public.notifications (user_id, type, actor_id, ref_id)
