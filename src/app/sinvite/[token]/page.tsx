...
import { SInviteClient } from './sinvite-client';

const loadInvite = loadServerInvite;
...
export default async function ServerInvitePage({ params }: { params: Promise<{ token: string }> }) {
  const { token } = await params;
  const inv = await loadInvite(token);
